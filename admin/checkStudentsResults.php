<?php
error_reporting(0);
session_start();
include("db_connection.php");
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <!-- ========== Meta Tags ========== -->
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="Avanni">

    <!-- ========== Page Title ========== -->
    <title>SRIT</title>

    <!-- ========== Favicon Icon ========== -->
    <link rel="shortcut icon" href="img/sritlogo.png" type="image/x-icon">

    <!-- ========== Start Stylesheet ========== -->
    <link rel="stylesheet" type="text/css" href="style.css">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" >
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"></script>
    <style type="text/css">
         .nav{
            
            list-style-type: none;
        }
        .nav{
              display: flex;
              justify-content:space-around;
              /*align-items: center;*/
              
              position: relative;top: 40%;
              height: 40px;
              line-height: 40px;
        }
    </style>
</head>
<body>

<div class="container pt-2">
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-6 col-6">
                <img src="sritlogo.png" alt="Logo" class="float-left col-lg-3 col-md-3 col-sm-3 col-5">
            </div>
            <div class="col-lg-6 col-md-6 col-sm-6 col-6">
                 <ul class="nav">
                    <li><a href="addQuestions.php">Add questions</a></li>
                    <li><a href="checkStudentsResults.php">performance</a></li>
                    <li><a href="labdate.php">External lab Date</a></li>
                    <li><a href="logout.php">Logout</a></li>
                </ul>
            </div>
        </div>
    </div><br>
    <div class="pt-5">
        <h5 class="text-center text-success">Students completed list</h5>
    </div>
    <div class="container p-4">
        <div class="table-responsive">
            <table class="table table-hover text-center">
                <thead>
                    <tr>
                        <th>Name</th>
                        <th>Roll number</th>
                        <th>qns completed</th>
                        <th>Total score</th>
                    </tr>
                </thead>
                <tbody>
                   <?php

                        
                                   
                        $sq = $conn->prepare("select count(question_name) as total from add_qns");
                        $sq->execute();
                        $res = $sq->get_result();
                        $r = $res->fetch_assoc();
                        $qnsComCount = $r['total'];

                        $sql = $conn->prepare("select * from std_ans_qns");
                        $sql->execute();
                        $result = $sql->get_result();

                        while($row = $result->fetch_assoc())
                        {
                            $id = $row['std_id'];

                            $sql1 = $conn->prepare("select * from student_registration where std_id = ?");
                            $sql1->bind_param("i", $id);
                            $sql1->execute();
                            $result1 = $sql1->get_result();
                            $row1 = $result1->fetch_assoc();

                            $std_name =  $row1['name'];
                            $rollnumber = $row1['username'];

                            $sql2 = $conn->prepare("select count(std_id) as Comqns from std_ans_qns");
                            $sql2->execute();
                            $res1 = $sql2->get_result();
                            $r1 = $res1->fetch_assoc();
                            $Comqns = $r1['Comqns'];

                            $qns_id = $row['qns_id'];
                            
                            ?>
                            <tr>
                                <td><?php echo($std_name); ?></td>
                                <td><?php echo($rollnumber) ?></td>
                                <td><?php echo($qns_id) ?></td>
                                <td><?php echo ($Comqns." / ".$qnsComCount) ?></td>
                            </tr>

                            <?php
                        }

                   ?>
                </tbody>
            </table>
            
        </div>
    </div>


    <!-- jQuery Frameworks
    ============================================= -->
    <script src="assets/js/validation_js.js"></script>
    <script src="assets/js/jquery-1.12.4.min.js"></script>
    <script src="assets/js/popper.min.js"></script>
    <script src="assets/js/bootstrap.min.js"></script>
    <script src="assets/js/equal-height.min.html"></script>
    <script src="assets/js/jquery.appear.js"></script>
    <script src="assets/js/jquery.easing.min.js"></script>
    <script src="assets/js/jquery.magnific-popup.min.js"></script>
    <script src="assets/js/modernizr.custom.13711.js"></script>
    <script src="assets/js/owl.carousel.min.js"></script>
    <script src="assets/js/wow.min.js"></script>
    <script src="assets/js/progress-bar.min.js"></script>
    <script src="assets/js/isotope.pkgd.min.js"></script>
    <script src="assets/js/imagesloaded.pkgd.min.js"></script>
    <script src="assets/js/count-to.js"></script>
    <script src="assets/js/YTPlayer.min.js"></script>
    <script src="assets/js/jquery.nice-select.min.js"></script>
    <script src="assets/js/bootsnav.js"></script>
    <script src="assets/js/main.js"></script>

</body>
</html> 